...
class UbloxProcessorEventType(Enum):
    NAVCLOCK_EVENT = auto()
...
class UbloxProcessor():
    ...
    def __init__(self):
        self._last_message = {}
        self._listeners = {}
        for event_type in UbloxProcessorEventType:
    ...
    def _timebase(self, timebase, timeRefGnss, utcStandard):
        ...
        if timebase == 1:
            return self.utcStdMap[utcStandard] if utcStandard in self.utcStdMap else self.utcStdMap[-1]
        return ''
    ...
    def receive(self, ubx_msg):
        ...
        try:
            ...
            print("ERROR> Unable to convert to TimedUBXMessage.")
    ...
    def _nav_clock(self, tubx_msg):
        ...
        self._notify(UbloxProcessorEventType.NAVCLOCK_EVENT, msg)
    ...
    def _nav_sat(self, tubx_msg):
        ...
        for i in range(1, tubx_msg.numSvs+1):
            ...
            sat['sat'] = (self.gnssPrefixMap[sat['gnssId']] if sat['gnssId'] in self.gnssPrefixMap else 
                            self.gnssPrefixMap[-1]) + "{0:02}".format(sat['svId'])
            msg['sats'].append(sat)
        self._notify(UbloxProcessorEventType.NAVSAT_EVENT, msg)

    def _tim_tp(self, tubx_msg):
        if (tubx_msg.identity not in ('TIM-TP')):
            return
        TimedUBXMessage.week = tubx_msg.week
        lts = GnssTime.now()
        ...
        self._notify(UbloxProcessorEventType.TIMEPULSE_EVENT, msg)
    ...
    def _rmx_rawx(self, tubx_msg):
        ...
        for i in range(1, tubx_msg.numMeas+1):
            ...
            if (meas['svId'] == 0):
                ...
                sigcode = 10*meas['gnssId'] + meas['reserved2']
                meas['sig'] = self.sigMap[sigcode] if sigcode in self.sigMap else self.sigMap[998]
            msg['meas'].append(meas)
        self._notify(UbloxProcessorEventType.RAWX_EVENT, msg)
    ...
    def _tim_svin(self, tubx_msg):
        ...
        self._notify(UbloxProcessorEventType.SURVEYIN_EVENT, msg)
        
        if ('NAV-PVT' in self._last_message):
            if (msg['valid'] == 1) and ((self._last_message['NAV-PVT']['msg']).fixType == 5):
                self._notify(UbloxProcessorEventType.SURVEY_DONE, self._last_message['NAV-PVT']['msg'])
