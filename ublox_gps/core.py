...
class Field:
    ...
    def value(self, value):    
        ...
        #if (self._type in ['U1', 'I1', 'X1', 'RU1_3', 'CH'] and sz > 1) \
        #    or (self._type in ['U2', 'I2', 'X2', 'RU2_5'] and sz > 2) \
        #    or (self._type in ['U4', 'I4', 'X4', 'R4'] and sz > 4) \
        #    or (self._type in ['I8', 'R8'] and sz > 8):
        #    raise ValueError('The provided value "{0}" of length {1} bytes is longer the the field "{2}" type {3}'
        #                        .format(value, sz, self.name, self._type))
        #else:
        #    self.__value = value
...
class Parser:
    ...
    def receive_from(self, stream) -> namedtuple:
        ...
        print("{0} - {1} - [{2}]".format(msg_cls, msg_id, length))
        ...
        print("Read payload length " + str(len(buff)))
        ...
        print("Read checksum calculated {:x} {:x}, received {:x} {:x}".format(
        ...
        print("Message> " + str(msg_cls) + ": " + str(buff[4:]))
        msg = self.classes[msg_cls].parse(msg_id, buff[4:])
        print("Parsed msg")
        print(str(msg))
        return msg
